...
function AllReport({ match }) {
  ...
  useEffect(() => {
    metadataService.getAllReport()
      .then((x) => {
        ...
        if ((location.state) && location.state.equipment_id != "" && location.state.equipment_name != "") {
          const initialSearch = x.filter(metaDataItem => ((metaDataItem.equipment_id).toLowerCase()).includes((location.state.equipment_id)) && metaDataItem.active_flag == 'true')
  ...
  function deleteConfirm(id, active_flag) {
    ...
    metadataService.deleteReportdata(id, active_flag).then(() => {
        let tempData = metaDatas.map(x => {
            if (x.id === id) {
                if (x.active_flag == 'true') {
                    x.active_flag = 'false';
                } else {
                    x.active_flag = 'true'
                }
  ...
  return (
    <div>
      <div className="flex-container" >
        ...
        <h3 className="title-text">Report List</h3>
      </div>
          ...
          {metaDatas && metaDatas.map(metaData =>
            <tr key={metaData.id}>
              ...
              <td style={{ width: '6%', wordBreak: 'break-word', textAlign: "center"}}>
                <button className="btn btn-sm btn-primary mr-1 edit-button" onClick={() => window.open(`http://13.80.147.178:8082/media/${metaData.report_data}`, "_blank")} disabled={metaData.active_flag == 'true' ? false : true}>View</button>
