...
function List({ match }) {
  ...
  return (
    <div>
      ...
      <button className="go-navigate"><Link to="/metadata_archive" className="nav-item nav-link">MetaData Archive</Link></button>
      <table className="table table-striped" style={{ tableLayout: "fixed" }}>
        <thead>
          <tr>
            ...
            <th style={{ width: '5%', wordBreak: 'break-word', textAlign: "center" }}><button>Action</button></th>
          ...
          {metaDatas && metaDatas.map(metaData =>
            <tr key={metaData.id}>
              ...
              <td style={{ width: '5%', wordBreak: 'break-word', textAlign: "center"}}>
                <Link to={`${path}/edit/${metaData.id}`} params={{ metaDatas: metaDatas }} className="btn btn-sm btn-primary mr-1 edit-button">Edit</Link>
                <button onClick={() => deleteData(metaData.id)} className="btn btn-sm btn-danger btn-delete-user" disabled={metaData.isDeleting}>
                  {metaData.isDeleting
              ...
              <td style={{ width: '5%', wordBreak: 'break-word', textAlign: "center"}}>
                <Link onClick={() => {showReportModal(metaData.equipment_name); setEquipmentName(metaData.equipment_name); setMetadataID(metaData.id)}} className="btn btn-sm btn-primary mr-1 edit-button">Add</Link>
              ...
              {consumptionData && consumptionData.map(data =>
                <tr key={data.pk}>
                  ...
                  <td style={{ width: '20%', wordBreak: 'break-word', textAlign: "center" }}>
                    <Link onClick={() => {showReportModal(data.fields.equipment_name); setEquipmentName(data.fields.equipment_name); setMetadataID(data.pk); setServiceRepair(data.fields.service_repair)}} className="btn btn-sm btn-primary mr-1 edit-button">Add</Link>
