...
function LocationPage({ match, props }) {
  ...
  useEffect(() => {
    ...
    if (location.state != null && location.state.longtitude != undefined && location.state.latitude != undefined) {
      setLongitude(location.state.longtitude)
    ...
    metadataService.getAll()
      .then((x) => {
        setMetaDatas(x)
        setTempMetaDatas(x)
        if (location.state != null && location.state.equipment_name != undefined) {
          searchMaintance(location.state.equipment_name, x)
        }
      })
  }, []);
  ...
  function searchMaintance(searchKey, maintance) {
    setMetaDatas(maintance)
    setMetaDatas(metaDatas => metaDatas.filter(x => (x.equipment_name) && (x.equipment_name).toLowerCase() == searchKey.toLocaleLowerCase()));
  }
    ...
    return (
      <div>
        <div className="flex-container">
          <img src="/src/assets/location.png" className="title-image" />
