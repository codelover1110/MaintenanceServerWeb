  ...
  useEffect(() => {
    metadataService.getAllReport()
      .then((x) => {
        console.log(x)
        let printData;
        const initialSearch = x.filter(metaDataItem => ((metaDataItem.active_flag).toLowerCase()).includes(('true')))
        //const initialSearch = x.filter(metaDataItem => ((metaDataItem.active_flag).toLowerCase()).includes(('true') && (metaDataItem.service_repair == 'matadata')))
